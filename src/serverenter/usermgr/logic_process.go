    ...
    if u_mgr.canWork() {
         u_mgr.http.PUT("/user/:id", func (c *gin.Context) {
              ...
              if !u_mgr.canWork() {
                ...
                if err != nil {
                    c.JSON(400, gin.H{"status" : "转换id参数错误......"})
                    return;
                }
                ...
                if u_mgr.db.Modify(usr_pack) != nil {
...
func (u_mgr *UserManager) registerAddUserByID() {
    if u_mgr.canWork() {
         u_mgr.http.POST("/user/:id", func (c *gin.Context) {
                ...
                if u_mgr.db.Create(&usr).Error != nil {
                    c.JSON(405, gin.H{"status" : "操作数据库时发生错误"})
                    return;
                }
...
func (u_mgr *UserManager) registerQueryUserByID() {
    if u_mgr.canWork() {
         u_mgr.http.GET("/user/:id", func (c *gin.Context) {
                ...
                usr_slice, err := u_mgr.db.Search(usr_pack)
                if err != nil {
                     c.JSON(405, gin.H{"status" : "操作数据库时发生错误"})
                     return;
                }
                c.JSON(http.StatusOK, gin.H{"object" : usr_slice})
        })
    }
}
...
func (u_mgr *UserManager) canWork() bool {
    return u_mgr.http != nil && u_mgr.db != nil &&u_mgr. srv_flag
}
 ...
 */
func (u_mgr *UserManager) getUserPack(c *gin.Context, ids ...interface{}) (*UserQueryPack, error) {
    usr_pack := UserQueryPack{}
 ...
 */
func (u_mgr *UserManager) getUser(c *gin.Context, ids ...interface{}) (*User, error) {
    usr := User{}
    //获取id
    if len(ids) != 1 {
        var err error
